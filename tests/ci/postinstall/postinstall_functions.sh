#!/bin/bash
# Post-install functions extracted from the AUR/RPM/DEB install scripts.
# This is the single source of truth for post-install logic, tested by the
# CI framework and (in Phase 5) sourced by all packaging scripts.
#
# Every function takes explicit parameters instead of reading globals so
# that tests can inject fake paths.
#
# Functions that call chown/install -o are shimmed: if _PI_NO_CHOWN is set,
# ownership changes are skipped (needed for unprivileged test runs).
