...
EOF
        ...
        vm_exec "echo 'yes' > ~/.config/gnome-initial-setup-done" 2>/dev/null || true
        # Enable AppIndicator extension for system tray (Fcitx5 icon)
        # Use system-wide dconf override (gsettings requires a running dbus session)
        vm_exec "sudo mkdir -p /etc/dconf/db/local.d && sudo sh -c 'cat > /etc/dconf/db/local.d/01-appindicator << END
[org/gnome/shell]
